...
row_w = tf.get_variable(name="row_w", shape=[28, 32], initializer=tf.ones_initializer())
col_w = tf.get_variable(name="col_w", shape=[28, 32], initializer=tf.ones_initializer())
filter_w = tf.Variable(tf.random_normal([3,3,1,32], stddev=0.01))
...
bias = tf.get_variable(name="b", shape=[28, 32], initializer=tf.ones_initializer())
...
one = tf.ones([size[0], 28, 32, 1])
col_sum = tf.matmul(add, one)


re_transpose = tf.transpose(add, perm=[0,2,1,3])
