# -*- coding: utf-8 -*-
"""
Created on Mon Jan 29 19:24:09 2018
...
row_w = tf.get_variable(name="row_w", shape=[14], initializer=tf.ones_initializer())
col_w = tf.get_variable(name="col_w", shape=[14], initializer=tf.ones_initializer())
sum_row_w = tf.get_variable(name="sum_row_w", shape=[14, 10], initializer=tf.ones_initializer())
sum_col_w = tf.get_variable(name="sum_col_w", shape=[14, 10], initializer=tf.ones_initializer())
logit = tf.get_variable(name="logit", shape=[14*14, 10], initializer=tf.ones_initializer())
bias = tf.get_variable(name="b", shape=[14], initializer=tf.ones_initializer())

x_re_temp = tf.reshape(x, [-1, 28, 28, 1])
x_pool = tf.nn.max_pool(x_re_temp, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')
x_re = tf.reshape(x_pool, [-1, 14, 14])
...
col_sum_re = tf.reshape(col_sum, [-1, 14])
row_sum_re = tf.reshape(row_sum, [-1, 14])
