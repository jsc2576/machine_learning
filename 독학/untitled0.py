...
W_fully = tf.get_variable(name="W_fully", shape=[8*8*64,1000], initializer=tf.contrib.layers.xavier_initializer())
W_result = tf.get_variable(name="W_result", shape=[1000,10], initializer=tf.contrib.layers.xavier_initializer())

B_fully = tf.get_variable(name="B_fully", shape=[1000], initializer=tf.contrib.layers.xavier_initializer())
...
with tf.Session(config=config) as sess:
     ...
     for epoch in range(train_epochs):
          ...
          for i in range(total_batch):
               batch_xs = x_train[(i%total_batch)*batch_size:((i+1)%total_batch)*batch_size,]
               batch_ys = yt_squeeze[(i%total_batch)*batch_size:((i+1)%total_batch)*batch_size,]
     ...
     print("Accuracy:", accuracy.eval({X: x_test, Y: y_test_squeeze}))
     sess.close()
